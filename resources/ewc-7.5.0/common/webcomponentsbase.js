...
export default class WebComponentsBaseComponent extends HTMLElement {
    ...
    connectedCallback2 () {
        ...
        if ( this.parentNode != null && this.parentNode.nodeName.slice( 0, 4 ) !== "EXT-" && this.A.o[ "viewport" ] == false ) {
    ...
    newCreateProps ( properties ) {
        ...
        if ( this.fitToParent == "true" || this.fitToParent == true || this.fitToParent == "" ) {
            o.height = "100%";
        }
    ...
    doChildren ( me ) {
        ...
        for ( const child of this.children ) {
            if ( child.nodeName.slice( 0, 4 ) !== "EXT-" ) {
                var w;
        ...
        if ( me.parentNode != null && me.parentNode.nodeName.slice( 0, 4 ) === "EXT-" ) {
    ...
    setEvent ( eventparameters, o, me ) {
        o.listeners[ eventparameters.name ] = function () {
            const eventname = eventparameters.name;

            // console.log('in event: ' + eventname + ' ' + o.xtype)
            const parameters = eventparameters.parameters;
            const parms = parameters.split( "," );
