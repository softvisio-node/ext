...
Ext.define('Ext.calendar.dd.DaysAllDayTarget', {extend:'Ext.drag.Target', requires:['Ext.calendar.util.Dom', 'Ext.calendar.date.Range'], config:{view:null}, updateView:function(view) {
  ...
  var D = Ext.Date, view = info.view, event = info.event, index = Ext.calendar.util.Dom.getIndexPosition(info.positions, info.cursor.current.x), newStart = view.utcTimezoneOffset(D.add(view.dateInfo.full.start, D.DAY, index)), start = event.getStartDate(), before = newStart < start, difference = D.diff(before ? newStart : start, before ? start : newStart, D.DAY);
  if (before) {
...
Ext.define('Ext.calendar.dd.DaysBodyTarget', {extend:'Ext.drag.Target', config:{view:null}, updateView:function(view) {
  ...
  var view = this.getView(), proxy = info.source.getProxy(), w;
...
Ext.define('Ext.calendar.dd.WeeksTarget', {extend:'Ext.drag.Target', requires:['Ext.calendar.date.Range'], config:{view:null}, updateView:function(view) {
  ...
  var D = Ext.Date, cursor = info.cursor.current, view = info.view, cell = view.getCellByPosition(cursor.x, cursor.y), event = info.event, difference;
  difference = this.calculateDifference(event, view.getDateFromCell(cell), info.startDate);
  view.handleChange('drop', event, new Ext.calendar.date.Range(D.add(event.getStartDate(), D.DAY, difference, false), D.add(event.getEndDate(), D.DAY, difference, false)), function() {
    view.clearSelected();
  });
  this.callParent([info]);
}, destroy:function() {
...
Ext.define('Ext.calendar.header.Days', {extend:'Ext.calendar.header.Base', xtype:'calendar-daysheader', format:'D m/d', compactOptions:{format:'d'}, getElementConfig:function() {
  ...
  row.insertFirst({tag:'td', cls:me.$headerCls + ' ' + me.$gutterCls}, true);
  row.append({tag:'td', style:'display: none;'}, true);
  return cells;
...
Ext.define('Ext.calendar.store.Events', {extend:'Ext.data.Store', alias:'store.calendar-events', model:'Ext.calendar.model.Event', requires:['Ext.calendar.model.Event', 'Ext.calendar.date.Range'], config:{calendar:null, calendarParam:'calendar', dateFormat:'C', endParam:'endDate', prefetchMode:'month', startParam:'startDate'}, remoteSort:false, pageSize:0, sorters:[{direction:'ASC', sorterFn:function(a, b) {
  ...
  var me = this, D = Ext.Date, R = Ext.calendar.date.Range, current = me.range, prefetchSettings = me.getPrefetchSetting(), fetchCount = 0, isLeading = false, fetchStart, fetchEnd, fullStart, fullEnd, newRange;
  ...
  newRange = {actual:range.clone(), full:new R(fullStart, fullEnd)};
...
}, updateHighlightToday:Ext.emptyFn, getDisplayRange:function() {
...
Ext.define('Ext.calendar.store.EventSource', {extend:'Ext.data.Store', requires:['Ext.calendar.date.Range'], config:{source:null}, sorters:[{direction:'ASC', sorterFn:function(a, b) {
  ...
  var me = this, oldIndex = me.lastIndex, contained = me.lastIndex !== -1, contains = me.contains(record);
  me.resumeEvents();
...
header:null, store:null, timezoneOffset:undefined, value:undefined}, platformConfig:{'!desktop':{compact:true}}, twoWayBindable:{value:1}, constructor:function(config) {
  ...
  if (me.getAddForm()) {
    if (!event) {
      ...
      event = me.createModel({startDate:range.start, endDate:D.add(range.end, D.DAY, 1), allDay:true});
    }
...
0, 1), MS_TO_MINUTES:60000, minimumEventMinutes:30, slotTicks:5, slotsPerHour:null, backPosName:'left', forwardPosName:'right', headerScrollOffsetName:'padding-right', calculateSlots:function() {
  ...
  return new Ext.scroll.NativeScroller({element:this.bodyWrap, x:false, y:true});
  ...
  if (creating) {
    ...
    event = me.createModel({startDate:D.localToUtc(start), endDate:D.add(D.localToUtc(end), D.DAY, 1), allDay:true});
  ...
  me.isAllDayCreating = {positions:positions, initialIndex:index, startDate:D.add(start, D.DAY, index)};
  ...
  var me = this, scrollable = me.scrollable, name = me.headerScrollOffsetName, w;
  if (scrollable) {
...
Ext.define('Ext.calendar.panel.Days', {extend:'Ext.calendar.panel.Base', xtype:'calendar-days', requires:['Ext.calendar.header.Days', 'Ext.calendar.view.Days'], config:{dayHeader:{xtype:'calendar-daysheader'}, eventRelayers:{view:{beforeeventdragstart:true, validateeventdrop:true, eventdrop:true, beforeeventresizestart:true, validateeventresize:true, eventresize:true}}, view:{xtype:'calendar-daysview'}}, configExtractor:{dayHeader:{dayHeaderFormat:'format'}, view:{allowSelection:true, displayOverlap:true, 
draggable:true, droppable:true, endTime:true, resizeEvents:true, showNowMarker:true, startTime:true, timeFormat:true, visibleDays:true, highlightToday:true}}, setTimeRange:function(start, end) {
...
value:undefined, visibleDays:7, visibleWeeks:2, weekendDays:undefined}, constructor:function(config) {
  ...
  if (allowSelection) {
    me.el.on({destroyable:true, scope:me, touchstart:'onTouchStart', touchmove:'onTouchMove', drag:'onDrag', touchend:'onTouchEnd', longpress:'onLongPress'});
  }
}, onDrag:function(e) {
  var me = this;
  ...
  table.createChild({tag:'tbody', children:me.generateCells(me.dateInfo.requiredWeeks, true)}, null, true);
...
clearSelected:function() {
  ...
  return {full:new R(start, end), visible:new R(start, end), active:new R(start, D.subtract(end, D.DAY, 1)), requiredWeeks:visibleWeeks};
}, doRefresh:function() {
  var me = this, D = Ext.Date, dateInfo = me.dateInfo, dayFormat = me.getDayFormat(), weekendDayMap = me.weekendDayMap, now = D.clearTime(Ext.calendar.date.Util.getLocalNow()), current = me.utcToLocal(dateInfo.visible.start), classes = [], trackRanges = me.trackRanges, visibleDays = me.getVisibleDays(), daysInWeek = Ext.Date.DAYS_IN_WEEK, y = now.getFullYear(), m = now.getMonth(), d = now.getDate(), cells, len, i, cell, firstDate, lastDate, highlight;
  if (trackRanges) {
  ...
  for (i = 0, len = cells.length; i < len; ++i) {
    ...
    if (current.getFullYear() === y && current.getMonth() === m && current.getDate() === d) {
      ...
      highlight = me.highlightToday;
      if (highlight) {
        classes.push(me.$todayCellHighlightCls + '-weeks-day');
      }
    }
  ...
  me.hideOverflowPopup();
  me.refreshEvents();
}, hideOverflowPopup:Ext.privateFn, maybeShowAddForm:function(start, end) {
  var me = this, D = Ext.Date, event;
  if (me.getAddOnSelect()) {
    if (me.hasEditableCalendars() && me.getAddForm()) {
      event = me.createModel({allDay:true, startDate:D.localToUtc(start), endDate:D.add(D.localToUtc(end), D.DAY, 1)});
      me.showAddForm(event, {scope:me, onSave:me.clearSelected, onCancel:me.clearSelected});
    }
  }
}, onLongPress:function(e) {
  var me = this, cell = me.getCellFromEvent(e), date;
  if (!cell) {
    return;
  }
  date = me.getDateFromCell(cell);
  me.fireEvent('select', me, {date:date});
  ...
  var me = this, cells = me.cells, start, end, temp;
  if (me.isSelecting) {
  ...
  if (e.getTarget('.' + me.$overflowCls, el) || e.getTarget('.' + me.$overflowPopupCls, el)) {
    return;
  }
...
overflowText:true, showOverflow:true, visibleDays:true, visibleWeeks:true, weekendDays:true, highlightToday:true}}});
Ext.define('Ext.calendar.view.Month', {extend:'Ext.calendar.view.Weeks', xtype:'calendar-monthview', config:{value:undefined, visibleWeeks:6}, nextMonth:function(months) {
  ...
  this.navigate(-this.getNavigateValue(years), Ext.Date.YEAR);
}, privates:{displayRangeProp:'month', maxWeeks:6, $rowClasses:[Ext.baseCSSPrefix + 'calendar-month-4weeks', Ext.baseCSSPrefix + 'calendar-month-5weeks', Ext.baseCSSPrefix + 'calendar-month-6weeks'], $todayCellHighlightCls:Ext.baseCSSPrefix + 'today-cell-highlight', trackRanges:true, doRecalculate:function(start) {
  ...
  return {full:new R(start, end), visible:new R(start, end), active:new R(start, D.subtract(end, D.DAY, 1)), month:new R(first, last), requiredWeeks:requiredWeeks};
}, doRefresh:function() {
...
Ext.define('Ext.calendar.panel.Month', {extend:'Ext.calendar.panel.Weeks', xtype:'calendar-month', requires:['Ext.calendar.view.Month'], config:{view:{xtype:'calendar-monthview'}}, autoSize:false, nextMonth:function(months) {
  ...
  var view = this.getView(), header = view.getHeader(), cellEl, domFormat;
  domFormat = Ext.Date.format(new Date(), header.domFormat);
  cellEl = view.element.selectNode('td[data-date\x3d"' + domFormat + '"]', false);
  if (cellEl) {
    cellEl.toggleCls(view.$todayCellHighlightCls + '-weeks-day', highlightToday);
  }
}});
...
centered:false, enter:'left', exit:'left', hideOnMaskTap:true, stretchY:true, ui:'light', header:{border:false, title:'Calendars'}}, titleBar:{docked:'top'}}, items:[{xtype:'panel', reference:'mainContainer', flex:1, layout:'fit'}], autoSize:false, initialize:function() {
  var me = this, ct = me.lookup('mainContainer');
  if (!me.getCompact()) {
    me.addSideBar();
  }
  me.addTitleBar();
...
Ext.define('Ext.calendar.panel.Week', {extend:'Ext.calendar.panel.Days', xtype:'calendar-week', requires:['Ext.calendar.view.Week'], config:{view:{xtype:'calendar-weekview'}}, configExtractor:{view:{firstDayOfWeek:true}}, $todayCellHighlightCls:Ext.baseCSSPrefix + 'today-cell-highlight', updateDayHeader:function(dayHeader, oldDayHeader) {
  this.callParent(arguments);
  this.updateHighlightToday(this.extracted.view.highlightToday);
}, updateHighlightToday:function(highlightToday) {
  var view = this.getView(), header = view.getHeader(), headerEl, domFormat;
  domFormat = Ext.Date.format(new Date(), header.domFormat);
  headerEl = header.element.selectNode('td[data-date\x3d"' + domFormat + '"]', false);
  if (headerEl) {
    headerEl.toggleCls(this.$todayCellHighlightCls + '-day', highlightToday);
  }
}});
...
createButton:{xtype:'button', cls:Ext.baseCSSPrefix + 'calendar-panel-create-button', text:'Create'}, createButtonPosition:'sideBar', dateTitle:{xtype:'component', reference:'calTitle', cls:Ext.baseCSSPrefix + 'calendar-panel-title', margin:'0 0 0 10'}, highlightToday:true, nextButton:{xtype:'button', text:'\x3e'}, previousButton:{xtype:'button', text:'\x3c'}, sideBar:{xtype:'panel', cls:Ext.baseCSSPrefix + 'calendar-sidebar'}, store:null, switcher:{xtype:'segmentedbutton', reference:'switcher', 
cls:Ext.baseCSSPrefix + 'calendar-panel-switcher', allowMultiple:false}, switcherPosition:'titleBar', timezoneOffset:undefined, titleBar:{xtype:'toolbar'}, todayButton:{xtype:'button', text:'Today', margin:'0 10 0 0'}, value:undefined, views:{day:{xtype:'calendar-day', titleTpl:'{start:date("l F d, Y")}', controlStoreRange:false, label:'Day', weight:10, dayHeader:null}, week:{xtype:'calendar-week', dayHeaderFormat:'D d', controlStoreRange:false, titleTpl:'{start:date("j M")} - {end:date("j M Y")}', 
label:'Week', weight:20}, month:{xtype:'calendar-month', titleTpl:'{start:date("F Y")}', label:'Month', weight:30}}}, defaultView:'month', cls:Ext.baseCSSPrefix + 'calendar-panel', moveNext:function() {
  this.getView().moveNext();
}, movePrevious:function() {
  this.getView().movePrevious();
}, navigate:function(amount, interval) {
  ...
  var me = this;
  return {xtype:'calendar-multiview', reference:'view', compact:me.getCompact(), defaultView:me.defaultView, store:me.getStore(), timezoneOffset:me.autoOffset ? undefined : me.getTimezoneOffset(), value:me.getValue(), views:me.getViews(), highlightToday:me.getHighlightToday(), listeners:{scope:me, valuechange:'onValueChange'}};
}, doSetView:function(view, fromSwitcher) {
  if (!fromSwitcher) {
    this.setSwitcherValue(view);
    return;
  }
  this.getView().highlightToday = this.getHighlightToday();
  this.getView().setView(view);
  this.refreshCalTitle();
}, updateHighlightToday:function(highlightToday) {
  if (!this.isConfiguring) {
    this.getView().updateHighlightToday(highlightToday);
  }
